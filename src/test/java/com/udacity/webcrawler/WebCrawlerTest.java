...
.containsExactly(Map.entry("the", 4), Map.entry("jumped", 2), Map.entry("brown", 2))
.inOrder();
}
...
assertThat(result.getWordCounts().entrySet())
.containsExactly(Map.entry("brown", 2), Map.entry("quick", 2), Map.entry("lazy", 2))
.inOrder();
}
...
assertThat(result.getWordCounts().entrySet())
.containsExactly(Map.entry("the", 8), Map.entry("jumped", 4), Map.entry("brown", 4))
.inOrder();
}
}
